...
public class TwoButtonSeekBar extends View {
    ...
    private long max = 100;//max damge
    private long min = 0;//min damge
    private double the_low=0;//low
    private double the_height = 100;//height
   ...
   private Context context;
    ...
    public TwoButtonSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TwoButtonSeekBar(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
//        this.setBackgroundColor(Color.BLACK);
       this.context = context;
        Resources resources = getResources();
        notScrollBarBg = resources.getDrawable(R.mipmap.un);
        hasScrollBarBg = resources.getDrawable(R.mipmap.en);
        mThumbLow = resources.getDrawable(R.mipmap.postion);
        mThumbHigh = resources.getDrawable(R.mipmap.postion2);
...
//            Log.d(TAG, "onDraw-->mOffsetLow: " + mOffsetLow + "  mOffsetHigh: " + mOffsetHigh   + "  progressLow: " + progressLow + "  progressHigh: " + progressHigh);
        the_low = ((progressLow/100)*(max-min)+min);
        the_height =  ((progressHigh/100)*(max-min)+min);
        int m_0 = (int) (the_low/60/1000);
        int s_0 = (int) ((the_low - m_0*60*1000)/1000);
        int ms_0 = (int) (the_low - m_0*60*1000 - s_0*1000);
        int m_1 = (int) (the_height/60/1000);
        int s_1 = (int) ((the_height - m_1*60*1000)/1000);
        int ms_1 = (int) (the_height - m_1*60*1000 - s_1*1000);
        canvas.drawText(m_0+":" +s_0+ "."+ms_0, (int)mOffsetLow - 2 - 2, 15, text_Paint);
        canvas.drawText(m_1+":" +s_1+ "."+ms_1, (int)mOffsetHigh - 2, 15, text_Paint);

        if (mBarChangeListener != null) {
            if (!isEdit) {
                mBarChangeListener.onProgressChanged(this, the_low, the_height);
            }
...
//            Log.d(TAG, "onDraw-->mOffsetLow: " + mOffsetLow + "  mOffsetHigh: " + mOffsetHigh   + "  progressLow: " + progressLow + "  progressHigh: " + progressHigh);
    ...
    public void setProgressLow(double  progressLow) {
        progressLow = (progressLow-min)/(max-min)*100;
    ...
    public void setProgressHigh(double  progressHigh) {
        progressHigh = (progressHigh - min)/(max-min)*100;
    ...
    public interface OnSeekBarChangeListener {
        ...
        //滑动时
        public void onProgressChanged(TwoButtonSeekBar seekBar, double progressLow,
                                      double progressHigh);
