...
public class CheapWAV extends CheapSoundFile {
    ...
    public void ReadFile(File inputFile)
        ...
        if (header[0] != 'R' ||
            ...
            header[10] != 'V' ||
            header[11] != 'E') {
            throw new java.io.IOException("这不是一个WAV格式的音频，他不应该拥有 .wav 拓展名");
        }
    ...
    public void WriteFile(File outputFile, int startFrame, int numFrames)
        ...
        for (int i = 0; i < numFrames; i++) {
            if(startFrame+i>=mFrameLens.length)
                throw new IOException("数据越界 数据应当不超过 "+(mFrameLens.length-1));
